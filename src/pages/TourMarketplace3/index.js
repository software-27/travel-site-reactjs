...
const TourMarketplacePage4 = () => {
  ...
  const [ftData, setFtData] = useState(dataFt);
  ...
  useEffect(() => {
    const filteredData = dataFt.filter(product => {
  ...
  const totalPagesDeals = Math.ceil(ftData.length / productsPerPageDeals); // Calculate total pages
  ...
  const sortData = () => {
    // Check if ftData is an array
    // if (Array.isArray(ftData)) {
    switch (productSort) {
      case 'LTH':
        return ftData.slice().sort((a, b) => a.price - b.price);
      case 'HTL':
        return ftData.slice().sort((a, b) => b.price - a.price);
      case 'STL':
        return ftData.slice().sort((a, b) => parseDurationToMinutes(a.duration) - parseDurationToMinutes(b.duration));
      case 'LTS':
        return ftData.slice().sort((a, b) => parseDurationToMinutes(b.duration) - parseDurationToMinutes(a.duration));
      default:
        return ftData.slice(); // Default case, no sorting
    }
    // } else {
    //   // If ftData is not an array (object), return it as is
    //   return ftData;
    // }
  };
  ...
  ftDataDisplay = sortedData.slice(startIndexDeals, endIndexDeals);
  console.log("ftDataDisplay", ftDataDisplay);
            ...
            <div className="flex flex-col items-center justify-start w-full overflow-x-hidden">
              <div className="flex flex-col items-start justify-start max-w-[1388px] mt-[60px] mx-auto px-[40px] w-full">
                <div className="flex flex-row">
                  ...
                  >
                    TRAVEL CREATOR MARKETPLACE - FLIGHT & TRANSPORT
                  </TextMp>
                </div>
                <div>
                ...
                <div className="flex flex-row font-poppins items-center justify-between mt-[40px] w-full">
                  <ButtonMp
                    className={`hover:bg-[#D7EEEB] ${isButtonSelected('deals') ? 'bg-cyan-700 text-white' : 'bg-white text-green-900'
                  ...
                  ${searchText != '' ? 'cursor-not-allowed opacity-50' : ''
                    ...
                    onClick={() => handleTagChange('deals')}
                    disabled={searchText !== ''}
                  >
                    Deals
                  </ButtonMp>
                  {/* <ButtonMp
                    className={`hover:bg-[#D7EEEB] ${isButtonSelected('hotels') ? 'bg-cyan-700 text-white' : 'bg-white text-green-900'
                      } cursor-pointer font-semibold w-[18%] outline outline-[1px] outline-gray-400 py-8 rounded-[10px] shadow-bs1 text-center xl:text-[15px] xs:text-[10px] tracking-[-0.27px]
                  ${searchText != '' ? 'cursor-not-allowed opacity-50' : ''
                      }`}
                    onClick={() => handleTagChange('hotels')}
                    disabled={searchText != ''}
                  >
                    Hotels
                  </ButtonMp>
                    ...
                    <div className="flex flex-col items-center justify-start w-full">
                      ...
                      >
                        {!ftData ? (
                          ...
                          <>
                            <div>
                              {ftDataDisplay.map((product, index) => (
                                <div
                                  key={product.id}
