...
const TourMarketplacePage4 = () => {
  ...
  const [dealsData, setDealsData] = useState(dataDl);
  ...
  useEffect(() => {
    const filteredData = dataDl.filter(product => {
      ...
      return matchesSearchText && matchesPriceRange && matchesDuration && matchesRating && matchesTags;
    });

    setDealsData(filteredData);
  }, [searchText, minPrice, maxPrice, durationFilter, ratingFilter, selectedTags]);
  ...
  const totalPagesDeals = Math.ceil(dealsData.length / productsPerPageDeals); // Calculate total pages
  ...
  const sortData = () => {
    // Check if dealsData is an array
    // if (Array.isArray(dealsData)) {
    switch (productSort) {
      case 'LTH':
        return dealsData.slice().sort((a, b) => a.price - b.price);
      case 'HTL':
        return dealsData.slice().sort((a, b) => b.price - a.price);
      case 'STL':
        return dealsData.slice().sort((a, b) => parseDurationToMinutes(a.duration) - parseDurationToMinutes(b.duration));
      case 'LTS':
        return dealsData.slice().sort((a, b) => parseDurationToMinutes(b.duration) - parseDurationToMinutes(a.duration));
      default:
        return dealsData.slice(); // Default case, no sorting
    }
    // } else {
    //   // If dealsData is not an array (object), return it as is
    //   return dealsData;
    // }
  };
  ...
  let dealsDataDisplay = [];

  dealsDataDisplay = sortedData.slice(startIndexDeals, endIndexDeals);
  console.log("dealsDataDisplay", dealsDataDisplay);
            ...
            <div className="flex flex-col items-center justify-start w-full overflow-x-hidden">
              <div className="flex flex-col items-start justify-start max-w-[1388px] mt-[60px] mx-auto px-[40px] w-full">
                <div className="flex flex-row">
                  ...
                  >
                    TRAVEL CREATOR MARKETPLACE - DEALS
                  </TextMp>
                </div>
                <div>
                ...
                <div className="flex flex-row font-poppins items-center justify-between mt-[40px] w-full">
                  <ButtonMp
                    className={`hover:bg-[#D7EEEB] ${isButtonSelected('flight') ? 'bg-cyan-700 text-white' : 'bg-white text-green-900'
                  ...
                  ${searchText != '' ? 'cursor-not-allowed opacity-50' : ''
                    ...
                    onClick={() => handleTagChange('flight')}
                    disabled={searchText !== ''}
                  >
                    Flight
                  </ButtonMp>
                  <ButtonMp
                    className={`hover:bg-[#D7EEEB] ${isButtonSelected('accom') ? 'bg-cyan-700 text-white' : 'bg-white text-green-900'
                      } cursor-pointer font-semibold w-[18%] outline outline-[1px] outline-gray-400 py-8 rounded-[10px] shadow-bs1 text-center xl:text-[15px] xs:text-[10px] tracking-[-0.27px]
                  ${searchText != '' ? 'cursor-not-allowed opacity-50' : ''
                      }`}
                    onClick={() => handleTagChange('accom')}
                    disabled={searchText != ''}
                  >
                    Accommodation
                  </ButtonMp>
                  ...
                  ${searchText != '' ? 'cursor-not-allowed opacity-50' : ''
                    ...
                    onClick={() => handleTagChange('transport')}
                    disabled={searchText != ''}
                  >
                    Transport
                  </ButtonMp>
                  ...
                  <div className="flex flex-col gap-[21px] items-center justify-start w-[73%]">
                    ...
                    <div className="flex flex-col items-center justify-start w-full">
                      ...
                      >
                        {!dealsData ? (
                        ...
                        ) : dealsData.length === 0 && showSpinner ? (
                          ...
                          <>
                            <div>
                              {dealsDataDisplay.map((product, index) => (
                                <div
                                  key={product.id}
                                  ...
                                  <div className="flex flex-row gap-[33px] items-center justify-between w-[100%] pr-[20px]">
                                    <div className="h-[289px] relative w-[37%]">
                                      <Img
                                        ...
                                        src={product.image || "/images/no-img.jpg"}
                                      // src={
...
export default TourMarketplacePage4;
