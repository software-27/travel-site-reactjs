...
import ThreeDotPrivate from "components/ThreeDotPrivate/index";
import { data, fetchItineraryDetails, fetchItineraryDestination, fetchTravelPlan } from "data/data";
import { useNavigate } from "react-router";
import Alert from '@mui/material/Alert';
import AlertTitle from '@mui/material/AlertTitle';
import "./LoadingSpinner4.css";
...
const PrivateContent = () => {
  ...
  const fetchTravelPlan = async () => {
    try {
      const response = await axios.get(`https://halaltravel.ai/hv/api/chatgpt/user/itinerary/listcover/DRAFT/${userId}/${currentPage}`, {
        headers: {
  ...
  // console.log(`https://halaltravel.ai/hv/api/chatgpt/user/itinerary/coverimage/${data.itineraryImage}`);
  ...
  return (
    <>
      ...
      {/* <button onClick={() => showAlertMessage('success')}>Show Success Alert</button>
      <button onClick={() => showAlertMessage('error')}>Show Error Alert</button> */}

      {showAlert && (
        <Alert severity={alertType}>
          <AlertTitle>{alertType === 'success' ? 'Settings Updated' : 'Error'}</AlertTitle>
          {alertType === 'success'
            ? 'Successfully updated guide settings!'
                ...
                {travelPlan.map((data, index) => (
                  <div className="bg-white common-pointer md:w-full md:h-[630px] lg:w-full lg:h-[360px] 2xl:h-[420px] border shadow-lg rounded-lg hover:scale-105 duration-300">
                    <div className="relative" key={index}>
                      <button className="absolute right-1 cursor-pointer">
                        <ThreeDotPrivate
                          ...
                          description={data.itineraryDescription}
                          days={data.days}
                    ...
                    <div className="items-center sm:p-6 lg:p-3"
                      ...
                      //   title={filteredFood.desc}
                      >
                        {data.itineraryDescription.length > 60
                          ? `${data.itineraryDescription.substring(0, 60)}...`
                          : data.itineraryDescription}
                        {/* Get a bird’s-eye view over Kuala Lumpur */}
                        {/* {filteredFood.desc.length > 40 ? filteredFood.desc.substring(0, 40) + '...' : filteredFood.desc} */}
                      </p>
